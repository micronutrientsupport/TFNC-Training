...
# Loading the data
df <- read.csv("Week-9/Materials/data/example_data.csv")
...
df <- slice(df,c(1:10)) #This selects only the rows 1:10 to keep. 
...
df <- select(df, ID, comments, f_name, Energy.in.KCalories, Protein.in.grams, Retinol..mcg., Magnesium..Mg..in.mg, Iron..Fe..in.mg, Iodine..I..in.mg, NA..Sodium..in.mg, Calcium.in.mcg)
...
df <- mutate(df, CAmg = CAmcg/1000)
#df$CAmg <- df$CAmg/1000

#Then we need to remove the incorrect column
df <- select(df,-CAmcg)

write.csv(df, "Week-9/data/cleaned_example_dataset.csv")
